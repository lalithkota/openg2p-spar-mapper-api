...
from ..schemas import IdFaMappingError, IdFaMappingErrorType
...
class MapperService(BaseService):
    async def link(self, id_fa_mappings: list[IdFaMapping]):
        ...
        async with session_maker() as session:
            for id_fa_mapping in id_fa_mappings:
                try:
                    session.add(
                        IdFaMapping(
                            id_value=id_fa_mapping.id,
                            fa_value=id_fa_mapping.fa,
                    ...
                    if "Unique" in str(e):
                        id_fa_mapping_errors.append(
                            IdFaMappingError(
                                id=id_fa_mapping.id,
                                fa=id_fa_mapping.fa,
                                error_type=IdFaMappingErrorType.id_unique_violation,
                            )
                        )
